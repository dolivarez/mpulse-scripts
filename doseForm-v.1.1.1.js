...
(function () {
 ...
 /* ================= FORMVIEW CAPTURE ================= */
(function interceptFormViewNetwork() {
  if (window.__FORMVIEW_NET_HOOKED__) return;
  window.__FORMVIEW_NET_HOOKED__ = true;
  ...
  window.fetch = async (...args) => {
    ...
    try {
      ...
      if (typeof url === "string" && url.includes("GetFormViewData")) {
        const clone = res.clone();
        window.__MPULSE_FORMVIEW__ = await clone.json();
        refreshDosePersonnel();
        console.log("📡 FormView refreshed via fetch");
      }
    } catch {}
    return res;
  };
  ...
  XMLHttpRequest.prototype.send = function () {
    this.addEventListener("load", () => {
      try {
        if (this.__url?.includes("GetFormViewData")) {
          window.__MPULSE_FORMVIEW__ = JSON.parse(this.responseText);
          refreshDosePersonnel();
          console.log("📡 FormView refreshed via XHR");
        }
      } catch {}
    });
  ...
  console.log("✅ FormView network interceptor installed");
})();
  ...
  function lockDoseMemo() {
    ...
    console.log("🔒 Dose memo locked (structured data only)");
  }
  ...
  if (!editor) {
    console.warn("❌ High-dose message field not found:", HIGH_DOSE_MESSAGE_FIELD);
    return;
  }
  ...
  if (existing.includes(timestamp)) {
    console.log("ℹ High-dose message already logged");
    return;
  }

  const block = `
    <hr>
    <p><strong>⚠ HIGH DOSE FLAG</strong></p>
    <p><strong>Reported Dose:</strong> ${value} ${unit}</p>
    <p><strong>Threshold Exceeded:</strong> ≥ ${threshold} ${unit}</p>
  ...
  console.warn("⚠ High-dose message written to OBJ_818");
}
  ...
  return new Promise(resolve => {
    const check = () => {
      ...
      if (Date.now() - start > timeout) {
        console.warn("⚠ FormView not available in time");
        return resolve(null);
      }
...
function getPersonnelList() {
  ...
  const map = new Map(); // Code → employee (CANONICAL)
  ...
  function refreshDosePersonnel() {
    ...
    select.innerHTML = `<option value="">Dose applies to…</option>`;
...
function parseStructuredRecords() {
  const editor = findEditorByField("CustomFields.OBJ_845");
  if (!editor || typeof editor.getData !== "function") return null;
  ...
  // 3️⃣ 🔑 Decode HTML entities
  text = decodeHtmlEntities(text);

  // 4️⃣ Extract + parse JSON blocks
  return [...text.matchAll(
    ...
    .map(m => {
      try {
        ...
        console.warn("❌ Failed to parse structured JSON block", m[1]);
        return null;
      }
    })
    .filter(Boolean);
...
async function writeSummaryDoseToField(records) {
  ...
  if (!ok) {
    console.warn("❌ Failed to write Total Dose via Angular");
    return;
  }

  console.log("💾 Total Dose written and WILL SAVE:", value);
}
...
function setDevExtremeValue(fieldId, value) {
  ...
  console.log("🧠 Total Dose set + bound:", value);
  return true;
}
...
function activateFinancialTab() {
  ...
  console.warn("❌ Financial tab not found");
  return false;
}

  (function hardLockTotalDoseDx() {
  ...
  document.addEventListener("keydown", e => {
    ...
    if (blockedKeys.includes(e.key)) {
      ...
      console.warn("🔒 Hard-blocked key on Total Dose:", e.key);
  ...
  function attach() {
    ...
    inst.on("valueChanged", e => {
      ...
      // 🚨 Block ALL user clears (including delete/backspace)
      if (invalid && !isSystem) {
        console.warn("🔒 Blocked manual clear of Total Dose");
...
function setTotalDoseAngular(value) {
  ...
  const widget =
    el.querySelector(".dx-numberbox") ||
    el.querySelector(".dx-texteditor");
  ...
  // 🔒 RELOCK
  setTimeout(() => inst.option("disabled", true), 0);
...
await writeSummaryDoseToField(records);
  ...
  container.innerHTML = `
    <div style="margin-top:10px;font-weight:bold">📊 Accrued Dose</div>
              ...
              <td style="padding:4px">
                ${
                  e.corrections.length
                    ? `✏ Corrected (${e.corrections.length})`
                    : `✓ Original`
                }
              </td>
              <td style="padding:4px;text-align:right">
...
function attachDoseHeaderButtonWhenEditable() {
  ...
  console.log("➕ Dose button attached (editable state)");
}
...
(function interceptGetFormViewData() {
  ...
  window.GetFormViewData = function (...args) {
    ...
    setTimeout(() => {
      console.log("📡 FormView reloaded → refreshing dose personnel");
      refreshDosePersonnel();
    }, 500);
  ...
  console.log("✅ GetFormViewData intercepted");
})();
  ...
  window.SaveTabAndKey = async function (...args) {
    try {
      ...
      if (editor && !editor.readOnly) {
        activateFinancialTab();                 // 👈 KEY
        const records = parseStructuredRecords();
        if (records) await writeSummaryDoseToField(records);
      }
    } catch (e) {
      console.warn("❌ Failed to write summary dose before save", e);
    }

    return original.apply(this, args);
  };

  console.log("✅ SaveTabAndKey intercepted + Financial tab activation");
})();
...
setTimeout(() => {
  ...
  panel.innerHTML = `
    ...
    <select id="dosePersonSelect"
      style="width:100%;padding:6px;margin-bottom:6px">
      <option value="">Dose applies to…</option>
    </select>
  ...
  document.body.appendChild(dock);   // 👈 attach FIRST
  dock.style.display = "none";   // 🔴 REQUIRED
  // ================= ESC CLOSE (ONE-TIME) =================
  ...
  if (!personSelect) {
    console.warn("❌ dosePersonSelect not found inside panel");
  } else {
    (async () => {
  ...
  (function watchPersonnelTable() {
  const targetSelector = ".dx-datagrid"; // MPulse personnel grid
  let lastSig = "";
  ...
  setInterval(() => {
    ...
    if (sig && sig !== lastSig) {
      lastSig = sig;
      console.log("👥 Personnel table changed → refreshing dose list");
      refreshDosePersonnel();
    }
  }, 1000);
})();
  ...
  if (!opt || isNaN(value)) {
    msg.textContent = "⚠ Enter value and select a person.";
    return;
  }

  const editor = findEditorByField(MEMO_FIELDNAME);
  if (!editor) {
    msg.textContent = "❌ Memo field not found.";
    return;
  }
...
`);
  ...
  msg.textContent = "✅ JSON dose entry written.";
};
