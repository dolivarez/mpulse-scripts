(function () {
  ...
  function resetReviewUIState() {
    ...
    console.log("🧹 Review UI state reset");
  }
  ...
  function attachReviewButtonWhenEditable() {
    ...
    // ✅ Already attached
    if (label.querySelector(".maint-review-btn")) return;
  ...
  function startReviewEditWatcher() {
    if (window.__REVIEW_EDIT_WATCHER_RUNNING__) return;
    ...
    setInterval(() => {
      ...
      // New editor instance after navigation
      const editorChanged = editor !== lastEditor;
      if (editorChanged) {
        lastEditor = editor;
        lastReadOnly = editor.readOnly;
        // attempt attach/remove based on current state
        attachReviewButtonWhenEditable();
        return;
      }
  
      // Detect pencil toggle (readOnly flip)
  ...
  function applyReview() {
    ...
    // ✅ DEFER CKEditor mutation to avoid MPulse race condition
    setTimeout(() => {
      unlockEditor(editor);
      editor.setData(html);
      lockEditor(editor);
  ...
  (function interceptFormNavigationForReview() {
    ...
    window.GetFormViewData = function (...args) {
      ...
      setTimeout(() => {
        waitForReviewEditor(() => {
          attachReviewButtonWhenEditable();
          startReviewEditWatcher();
          console.log("🔁 Review button reattached after navigation");
        });
      }, 300);
    ...
    console.log("📡 Review UI hooked into GetFormViewData");
  })();
