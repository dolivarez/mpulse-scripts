  ...
  ">
    ...
    <div style="flex:1;display:flex;align-items:center;justify-content:flex-end;">
      <img
        ...
        style="height:24px;max-width:140px;object-fit:contain;"
      >
    </div>
  </div>
`;
...
function drawPdfFooter(pdf, pageWidth, pageHeight, pageNum, totalPages) {
  ...
  while (yOffset < imgHeight) {
    ...
    sliceCanvas.height =
      Math.min(
        canvas.height - (yOffset * canvas.width) / imgWidth,
  ...
  const title = `${type.toUpperCase()} ${reportConfig.label.toUpperCase()} REPORT — ${data.id}`;

  // Headers + footers (drawn AFTER slicing)
