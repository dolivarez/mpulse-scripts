(function () {
  ...
  function getCurrentStatus() {
    const wrapper = document.querySelector(".OBJ_879");
    if (!wrapper || !window.$) return "NOT_REVIEWED";
  
    const dxElement =
        wrapper.querySelector(".dx-textbox") ||
        wrapper.querySelector(".dx-texteditor");
  
    if (!dxElement) return "NOT_REVIEWED";
  
    const instance = $(dxElement).dxTextBox("instance");
    if (!instance) return "NOT_REVIEWED";
  
    return instance.option("value") || "NOT_REVIEWED";
  }
  ...
  function setStatus(value) {
    ...
    if (!dxElement) {
      console.warn("Status dx element not found");
      return;
    }
  
    const instance = $(dxElement).dxTextBox("instance");
    if (!instance) {
      console.warn("dxTextBox instance not found");
      return;
    }
  
    // 🔓 Temporarily unlock if needed
    const wasReadOnly = instance.option("readOnly");
    if (wasReadOnly) instance.option("readOnly", false);
  
    // ✅ Set via DevExtreme API (this is key)
    instance.option("value", value);
  
    // 🔁 Trigger Angular digest
    const scope =
        angular.element(wrapper).scope() ||
        angular.element(wrapper).isolateScope();
  
    if (scope) scope.$applyAsync();
  
    // 🔒 Restore readOnly
    if (wasReadOnly) instance.option("readOnly", true);
  
    console.log("🟢 Status updated via dxTextBox API:", value);
  }
