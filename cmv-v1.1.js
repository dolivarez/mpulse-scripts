/* ============================================================
 * CMV v1.1 — Custom Media Viewer for MPulse
...
(() => {
  ...
  /* ------------------------------------------------------------
   * Record refresh state (MUST be defined early)
   * ------------------------------------------------------------ */
  let __cmvLastRecordSig = null;
  let __cmvRefreshQueued = false;
  ...
  function classifyMedia(rawMedia) {
    ...
    rawMedia.forEach((m) => {
      ...
      const item = {
        ...
        downloadUrl: isExternalLink
          ? null
          : buildDownloadUrl(file, key, token),
      
        viewerUrl: isExternalLink
  ...
  const mo = new MutationObserver(() => {
    injectToggle();
    queueRefreshOnContextChange();
  });
  mo.observe(document.body, { childList: true, subtree: true });
  
  injectToggle();
  queueRefreshOnContextChange(); // initialize signature
  ;
  ...
  function ensureLightbox() {
    ...
    main.addEventListener("wheel", (e) => {
      ...
      if (!item || !["image", "video"].includes(item.kind)) return;
      ...
      const newZoom = Math.min(
        ZOOM_MAX,
        Math.max(ZOOM_MIN, state.zoom * zoomFactor)
      );
      ...
      state.panX = cx - scaleRatio * (cx - state.panX);
      state.panY = cy - scaleRatio * (cy - state.panY);
  ...
  function renderLightbox(skipFade) {
    ...
    if (item.kind === "image") {
      ...
      window.open(item.viewerUrl, "_blank", "noopener,noreferrer");
      closeLightbox();
      return;
    } else {
  ...
  async function downloadCurrent() {
    ...
    try {
      showProgress(`Downloading: ${item.desc}`, 0);
  
      const blob = await fetchWithProgress(item.downloadUrl, (p) => {
        showProgress(`Downloading: ${item.desc}`, Math.round(p * 100));
      });
  ...
  function getRecordSignature() {
    // Prefer stable record ID if available; fallback to WOID + Token
    const wo = getWOID();
    const tok = ensureToken() || "";
    return `${wo}::${tok}`;
  }
  
  function isCustomViewerVisible() {
    const panel = document.getElementById(PANEL_ID);
    return panel && panel.style.display !== "none";
  }
  
  function resetForNewRecord() {
    // Clear cached state so classifyMedia forces fresh token + fresh items
    GLOBAL_TOKEN = null;
    MEDIA_ITEMS = [];
  
    // If lightbox is open, close it (avoids “next/prev” indexing into old record)
    const lb = document.getElementById(LB_ID);
    if (lb && lb.style.display === "flex") closeLightbox();
  }
  
  function refreshCustomPanelIfVisible() {
    const pane = findMediaPane();
    if (!pane) return;
    ...
    const wasVisible = existingPanel.style.display !== "none";
  
    const newPanel = buildPanel();
    newPanel.style.display = wasVisible ? "" : "none";
  ...
  function queueRefreshOnContextChange() {
    if (__cmvRefreshQueued) return;
    __cmvRefreshQueued = true;
  
    requestAnimationFrame(() => {
      __cmvRefreshQueued = false;
  
      const recordSig = getRecordSignature();
      const mediaSig  = getMediaSignature();
  
      const recordChanged = recordSig !== __cmvLastRecordSig;
      const mediaChanged  = mediaSig  !== __cmvLastMediaSig;
  
      if (!recordChanged && !mediaChanged) return;
  
      __cmvLastRecordSig = recordSig;
      __cmvLastMediaSig  = mediaSig;
  
      // Let Angular finish updating media DOM after upload
      setTimeout(() => {
        resetForNewRecord();          // clears MEDIA_ITEMS + token cache
        refreshCustomPanelIfVisible();
      }, 150);
    });
  }


  let __cmvLastMediaSig = null;

  function getMediaSignature() {
    try {
      const raw = extractRawMedia();
      if (!raw || !raw.length) return "empty";
  
      // Stable signature: keys + filenames
      return raw
  ...
  function showProgress(text, percent = 0) {
    let modal = document.getElementById("cmv-progress-v3");
    if (!modal) {
      modal = document.createElement("div");
      modal.id = "cmv-progress-v3";
      modal.innerHTML = `
        <div style="
          background:#222;
          padding:20px;
          ...
          text-align:center;
        ">
          <div id="cmv-progress-text-v3" style="margin-bottom:10px;">
            ${text}
          </div>
          <div style="background:#444;height:8px;border-radius:4px;overflow:hidden">
            <div id="cmv-progress-bar-v3"
                 style="width:${percent}%;height:100%;background:#00aaff"></div>
          </div>
          <div id="cmv-progress-pct-v3"
      ...
      modal.style.display = "flex";
      modal.querySelector("#cmv-progress-text-v3").innerText = text;
      modal.querySelector("#cmv-progress-bar-v3").style.width = percent + "%";
      modal.querySelector("#cmv-progress-pct-v3").innerText = percent + "%";
    }
  }
  
  function hideProgress() {
    const modal = document.getElementById("cmv-progress-v3");
  ...
  async function downloadZip(items, zipName, JSZip) {
    ...
    for (const item of items) {
      ...
      showProgress(
        `Downloading ${completed} / ${items.length}: ${item.desc}`,
        Math.round(((completed - 1) / items.length) * 100)
      );
  
      try {
        const blob = await fetchWithProgress(item.downloadUrl, (p) => {
          const base = (completed - 1) / items.length;
          const pct = Math.round((base + p / items.length) * 100);
          showProgress(
            `Downloading ${completed} / ${items.length}: ${item.desc}`,
            pct
          );
    ...
    showProgress("Finalizing ZIP…", 100);
