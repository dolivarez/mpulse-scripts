(function () {
  ...
  function syncButton() {
    ...
    const emailButton =
      target.querySelector("#toolbarEquipmentEmailButton");
    
    const emailItem =
      emailButton?.closest("li");
    
    const pdfItem =
      target.querySelector("#generateReportBtn");
    
    const moreDropdown =
      target.querySelector(".scheduled_dropdown");
    
    const moreItem =
      moreDropdown?.closest("li");
    
    if (emailItem) {
      // Preferred:
      // More | Create Request | Email | PDF
