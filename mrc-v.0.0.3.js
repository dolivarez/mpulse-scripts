(function () {
  ...
  function setReviewStatusValue(statusText) {

    const fieldEl = document.querySelector(
      '#OBJ_873[fieldname="CustomFields.OBJ_873"]'
    );
  
    if (!fieldEl) {
      console.warn("⚠ Status field element not found");
      return;
    }
  
    const ngScope = angular.element(fieldEl).scope();
  
    if (!ngScope || !ngScope.selectedRecord) {
      console.warn("⚠ Angular scope not found");
      return;
    }
  
    // Write directly to model
    ngScope.selectedRecord.CustomFields.OBJ_873 = statusText;
  
    // Trigger Angular digest safely
    try {
      ngScope.$applyAsync();
    } catch (err) {
      console.warn("⚠ $applyAsync failed", err);
    }
  
    console.log("✅ Review Status set via Angular model:", statusText);
  }
  ...
  function applyReview() {
    ...
    setReviewStatusValue(selectedStatus);
