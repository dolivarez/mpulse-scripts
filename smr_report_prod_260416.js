(function () {
  async function init() {
    console.log("📄 toolbarReportButton-SMR.js initialized");

    // =========================
    // CONFIG
    // =========================
    const HTML2CANVAS_SRC = "https://YOUR_HOST/html2canvas.min.js";
    const JSPDF_SRC = "https://YOUR_HOST/jspdf.umd.min.js";

    // =========================
    // HELPERS
    // =========================
    const resolve = (v) => {
      if (v == null || v === "") return "—";
      if (typeof v === "object") return v.Desc || v.Description || v.Code || v.ID || "—";
      return String(v);
    };

    const fmtDate = (v) => {
      if (!v || String(v).startsWith("0001")) return "—";
      const dt = new Date(v);
      return isNaN(dt) ? String(v) : dt.toLocaleString();
    };

    const safeFilename = (s) =>
      (s || "SMR_Report")
        .toString()
        .replace(/[^\w\-]+/g, "_")
        .replace(/_+/g, "_")
        .replace(/^_|_$/g, "");

    const esc = (s) =>
      String(s ?? "")
        .replace(/&/g, "&amp;")
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;");

    const waitForToolbar = () =>
      new Promise((resolve) => {
        const check = () => {
          const el = document.querySelector(".action-menu-items ul.itemDetailActionBtns");
          if (el) return resolve(el);
          requestAnimationFrame(check);
        };
        check();
      });

    const detectRecordType = () => {
      const el = document.querySelector(".module-name");
      return el?.textContent?.trim().toLowerCase() || "unknown";
    };

    const isSmrRecord = () => {
      const type = detectRecordType();
      if (type.includes("scheduled maintenance")) return true;
      if (type.includes("maintenance schedule")) return true;
      if (document.querySelector("#ScheduleType") || document.querySelector("#NextDueDate")) return true;
      return false;
    };

    const getArray = (obj, names) => {
      for (const name of names) {
        if (Array.isArray(obj?.[name])) return obj[name];
      }
      return [];
    };

    async function loadScriptOnce(src, globalCheck) {
      if (globalCheck()) return;

      await new Promise((resolve, reject) => {
        const existing = [...document.scripts].find(s => s.src === src);
        if (existing) {
          existing.addEventListener("load", resolve, { once: true });
          existing.addEventListener("error", reject, { once: true });
          return;
        }

        const s = document.createElement("script");
        s.src = src;
        s.onload = resolve;
        s.onerror = reject;
        document.head.appendChild(s);
      });
    }

    // =========================
    // CAPTURE / INTERCEPTOR
    // =========================
    (function installMpulseCapture() {
      if (window.__MPULSE_CAPTURE_INSTALLED__) return;
      window.__MPULSE_CAPTURE_INSTALLED__ = true;

      function tryStore(url, text) {
        try {
          const data = JSON.parse(text);

          if (/GetFormViewData/i.test(url)) {
            window.__MPULSE_FORMVIEW__ = data;
            console.log("Stored __MPULSE_FORMVIEW__", data);
            return;
          }

          if (/MoreViewClick/i.test(url)) {
            window.__MPULSE_MOREVIEW__ = data;
            console.log("Stored __MPULSE_MOREVIEW__", data);
            return;
          }

          if (/Load/i.test(url)) {
            window.__MPULSE_LOAD_RESPONSE__ = data;
            console.log("Stored __MPULSE_LOAD_RESPONSE__", data);
            return;
          }
        } catch (e) {
          // ignore non-JSON
        }
      }

      const origXHROpen = XMLHttpRequest.prototype.open;
      const origXHRSend = XMLHttpRequest.prototype.send;

      XMLHttpRequest.prototype.open = function (method, url, ...rest) {
        this.__url = url;
        this.__method = method;
        return origXHROpen.call(this, method, url, ...rest);
      };

      XMLHttpRequest.prototype.send = function (...args) {
        this.addEventListener("load", function () {
          try {
            const url = this.__url || "";
            if (/GetFormViewData|MoreViewClick|Load/i.test(url)) {
              tryStore(url, this.responseText);
            }
          } catch (e) {
            console.warn("XHR capture failed", e);
          }
        });

        return origXHRSend.apply(this, args);
      };

      if (!window.__MPULSE_ORIG_FETCH__) {
        window.__MPULSE_ORIG_FETCH__ = window.fetch;
      }

      window.fetch = async function (...args) {
        const res = await window.__MPULSE_ORIG_FETCH__.apply(this, args);

        try {
          const url = typeof args[0] === "string"
            ? args[0]
            : (args[0]?.url || "");

          if (/GetFormViewData|MoreViewClick|Load/i.test(url)) {
            const clone = res.clone();
            const text = await clone.text();
            tryStore(url, text);
          }
        } catch (e) {
          console.warn("Fetch capture failed", e);
        }

        return res;
      };

      console.log("MPulse capture installed");
    })();

    // =========================
    // REPORT TYPES
    // =========================
    const REPORT_TYPES = {
      summary: {
        label: "Summary",
        sections: ["general", "woInfo"],
        filenameSuffix: "Summary"
      },
      operational: {
        label: "Operational",
        sections: ["general", "generationRules", "woInfo", "comments"],
        filenameSuffix: "Operational"
      },
      technical: {
        label: "Technical",
        sections: ["general", "scheduleMeta", "generationRules", "woInfo", "comments"],
        filenameSuffix: "Technical"
      },
      full: {
        label: "Full",
        sections: ["general", "scheduleMeta", "generationRules", "woInfo", "comments"],
        filenameSuffix: "Full"
      }
    };

    async function chooseReportType() {
      return new Promise((resolve) => {
        const overlay = document.createElement("div");
        overlay.style.cssText = `
          position:fixed; inset:0; background:rgba(0,0,0,0.4);
          display:flex; align-items:center; justify-content:center; z-index:10000;
        `;

        const box = document.createElement("div");
        box.style.cssText = `
          background:#fff; padding:20px; border-radius:12px; min-width:320px;
          box-shadow:0 10px 30px rgba(0,0,0,0.25); font-family:Arial,sans-serif;
        `;

        box.innerHTML = `
          <h3 style="margin:0 0 12px 0;">Select SMR Report Type</h3>
          <div style="display:flex;flex-direction:column;gap:8px;">
            <button data-type="summary" style="padding:10px;border-radius:8px;cursor:pointer;">Summary</button>
            <button data-type="operational" style="padding:10px;border-radius:8px;cursor:pointer;">Operational</button>
            <button data-type="technical" style="padding:10px;border-radius:8px;cursor:pointer;">Technical</button>
            <button data-type="full" style="padding:10px;border-radius:8px;cursor:pointer;">Full</button>
          </div>
          <div style="margin-top:10px;text-align:right;">
            <button id="cancelReportType" style="padding:8px 12px;border-radius:8px;cursor:pointer;">Cancel</button>
          </div>
        `;

        box.querySelectorAll("button[data-type]").forEach(btn => {
          btn.onclick = () => {
            overlay.remove();
            resolve(btn.getAttribute("data-type"));
          };
        });

        box.querySelector("#cancelReportType").onclick = () => {
          overlay.remove();
          resolve(null);
        };

        overlay.appendChild(box);
        document.body.appendChild(overlay);
      });
    }

    function section(title, content) {
      return `
        <h3 style="margin-top:20px;padding-bottom:6px;border-bottom:2px solid #0078d7;">
          ${title}
        </h3>
        ${content}
      `;
    }

    function buildHeader(data, reportConfig) {
      const created = new Date().toLocaleString();
    
      return `
        <div style="
          display:flex;
          align-items:flex-start;
          justify-content:space-between;
          border-bottom:2px solid #0078d7;
          padding-bottom:10px;
          margin-bottom:12px;
          gap:12px;
        ">
    
          <!-- LEFT: SOFIE LOGO -->
          <div style="flex:0 0 140px;">
            <img src="data:image/png;base64,"
            style="height:52px; width:auto;">
          </div>
    
          <!-- CENTER: TITLE -->
          <div style="flex:1;">
            <div style="font-size:20px;font-weight:700;color:#222;">
              Scheduled Maintenance Report
            </div>
    
            <div style="font-size:13px;color:#555;margin-top:4px;">
              ${reportConfig?.label || "SMR"} Report
            </div>
    
           <!-- <div style="margin-top:6px;font-size:12px;color:#444;">
              ${esc(data.desc || "")}
            </div> -->
          </div>
    
          <!-- RIGHT: META + MPULSE LOGO -->
          <div style="
            flex:0 0 220px;
            text-align:right;
            font-size:12px;
            color:#444;
            line-height:1.4;
          ">
            <div style="margin-bottom:6px;">
              <img src="https://mpulse9.com//Media/GetMenuImage?DBName=SOFIE&ImageType=LOGOLG&ImageName=Sofie%20Logo.png" style="max-width:120px; height:auto;" />
            </div>
    
            <div><strong>SMR ID:</strong> ${esc(data.id)}</div>
            <div><strong>Last Updated:</strong> ${esc(data.lastUpdatedDate || "—")}</div>
            <div><strong>Report Created:</strong> ${esc(created)}</div>
          </div>
    
        </div>
      `;
    }

    function buildSimpleTable({ headers, rows }) {
      if (!rows || !rows.length) return `<div style="color:#888;">—</div>`;

      return `
        <div style="overflow:auto;border:1px solid #ccc;border-radius:6px;margin-top:6px;">
          <table style="border-collapse:collapse;width:100%;font-size:12px;table-layout:fixed;">
            <thead style="background:#f0f0f0;">
              <tr>
                ${headers.map(h => `
                  <th style="text-align:left;padding:6px;border-bottom:1px solid #ccc;">
                    ${esc(h)}
                  </th>
                `).join("")}
              </tr>
            </thead>
            <tbody>
              ${rows.map(r => `
                <tr>
                  ${r.map(c => `
                    <td style="padding:6px;border-bottom:1px solid #eee;vertical-align:top;">
                      ${esc(c)}
                    </td>
                  `).join("")}
                </tr>
              `).join("")}
            </tbody>
          </table>
        </div>
      `;
    }

    function getSmrData() {
      const d = window.__MPULSE_FORMVIEW__ || {};

      const enabled =
        typeof d.DisableSchedulerControl === "boolean"
          ? (d.DisableSchedulerControl ? "No" : "Yes")
          : "—";

      return {
        id: resolve(d.RecordId),
        desc: resolve(d.RecordDescription),
        tag: resolve(d.RecordTag),
        location: resolve(d.Locations),
        department: resolve(d.Departments),
        createdBy: resolve(d.CreatedBy),
        createdDate: fmtDate(d.CreatedDate),
        lastUpdatedBy: resolve(d.LastUpdatedBy),
        lastUpdatedDate: fmtDate(d.LastUpdatedDate),
        enabled,
        conversionLeadTime: resolve(d.ConversionLeadTime),
        actualHours: resolve(d.ActualHours),
        estHours: resolve(d.EstimatedHours),
        employeeCost: resolve(d.EmployeeCost),
        estCost: resolve(d.EstCost),
        otherCost: resolve(d.OtherCost),
        partCost: resolve(d.PartCost),
        comments: d.Comments || ""
      };
    }

    function buildSmrWoHierarchy() {
      const d = window.__MPULSE_FORMVIEW__ || {};
      const taskList = Array.isArray(d.ObjTaskList) ? d.ObjTaskList : [];
      const tasks = taskList.length
        ? taskList
        : (d.ObjCurrentTask ? [d.ObjCurrentTask] : []);

      if (!tasks.length) {
        return `<div style="color:#888;">No WO Information found</div>`;
      }

      return tasks.map((task, taskIndex) => {
        const taskCode = resolve(task.Code);
        const taskDesc = resolve(task.TskDesc || task.Description);
        const assets = getArray(task, ["AssetList", "ListOfAssets"]);

        const taskPersonnelCount = assets.reduce((sum, asset) => {
          return sum + getArray(asset, ["ListOfEmployee", "ListofEmployee", "PersonnelList"]).length;
        }, 0);

        const taskInventoryCount = assets.reduce((sum, asset) => {
          return sum + getArray(asset, ["ListOfInventory", "InventoryList", "ListOfLinkedInventories"]).length;
        }, 0);

        const assetsHtml = assets.length
          ? assets.map((asset, assetIndex) => {
              const assetId = resolve(asset.RecordId || asset.Code || asset.ID);
              const assetDesc = resolve(asset.RecordDescription || asset.Description || asset.Desc);

              const personnel = getArray(asset, ["ListOfEmployee", "ListofEmployee", "PersonnelList"]);
              const inventory = getArray(asset, ["ListOfInventory", "InventoryList", "ListOfLinkedInventories"]);

              const personnelTable = buildSimpleTable({
                headers: ["Personnel ID", "Name"],
                rows: personnel.map(p => [
                  resolve(p.Code || p.ID),
                  resolve(p.Description || p.Desc)
                ])
              });

              const inventoryTable = buildSimpleTable({
                headers: ["Inventory ID", "Description", "Quantity", "Units", "Stock Area"],
                rows: inventory.map(inv => [
                  resolve(inv.Code || inv.ID),
                  resolve(inv.Description || inv.Desc),
                  resolve(inv.Quantity),
                  resolve(inv.Units),
                  resolve(inv.StockArea)
                ])
              });

              return `
                <details class="smr-asset-block" style="
                  margin-top:12px;
                  border:1px solid #d9d9d9;
                  border-radius:8px;
                  background:#fafafa;
                  overflow:hidden;
                ">
                  <summary style="
                    list-style:none;
                    cursor:pointer;
                    padding:12px;
                    font-weight:700;
                    color:#333;
                    display:flex;
                    justify-content:space-between;
                    align-items:flex-start;
                    gap:12px;
                  ">
                    <div>
                      <div>Asset ${assetIndex + 1}: ${esc(assetId)}</div>
                      <div style="margin-top:4px;font-weight:400;color:#444;">
                        ${esc(assetDesc)}
                      </div>
                    </div>

                    <div style="
                      display:flex;
                      gap:12px;
                      flex-wrap:wrap;
                      font-size:12px;
                      color:#555;
                      font-weight:400;
                      text-align:right;
                    ">
                      <div><strong>Personnel:</strong> ${personnel.length}</div>
                      <div><strong>Inventory:</strong> ${inventory.length}</div>
                    </div>
                  </summary>

                  <div style="padding:0 12px 12px 12px;">
                    <div style="margin-bottom:12px;">
                      <div style="font-weight:600;margin-bottom:4px;">Personnel</div>
                      ${personnelTable}
                    </div>

                    <div>
                      <div style="font-weight:600;margin-bottom:4px;">Inventory</div>
                      ${inventoryTable}
                    </div>
                  </div>
                </details>
              `;
            }).join("")
          : `<div style="color:#888;margin-top:8px;">No assets linked to this task</div>`;

        return `
            <div class="smr-task-block" style="
              margin-top:${taskIndex === 0 ? 0 : 16}px;
              padding:14px;
              border:1px solid #cfd8dc;
              border-left:5px solid #0078d7;
              border-radius:8px;
              background:#fff;
            ">
            <div style="font-size:15px;font-weight:700;color:#222;">
              Task ${taskIndex + 1}: ${esc(taskCode)}
            </div>

            <div style="margin-top:4px;color:#444;">
              ${esc(taskDesc)}
            </div>

            <div style="
              display:flex;
              gap:16px;
              flex-wrap:wrap;
              font-size:12px;
              color:#555;
              margin-top:8px;
              margin-bottom:10px;
              padding:8px 10px;
              background:#eef4fb;
              border-radius:6px;
            ">
              <div><strong>Assets:</strong> ${assets.length}</div>
              <div><strong>Personnel:</strong> ${taskPersonnelCount}</div>
              <div><strong>Inventory:</strong> ${taskInventoryCount}</div>
            </div>

            ${assetsHtml}
          </div>
        `;
      }).join("");
    }

    function buildSections(data) {
      const general = `
        <div style="margin-top:12px;padding:10px;background:#f5f5f5;border-left:4px solid #0078d7;border-radius:4px;">
          <strong>Description:</strong><br>
          ${esc(data.desc)}
        </div>

        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:12px;">
          <div><strong>SMR ID:</strong> ${esc(data.id)}</div>
          <div><strong>Enabled:</strong> ${esc(data.enabled)}</div>

          <div><strong>Location:</strong> ${esc(data.location)}</div>
          <div><strong>Department:</strong> ${esc(data.department)}</div>

          <div><strong>Created By:</strong> ${esc(data.createdBy)}</div>
          <div><strong>Created Date:</strong> ${esc(data.createdDate)}</div>

          <div><strong>Last Updated By:</strong> ${esc(data.lastUpdatedBy)}</div>
          <div><strong>Record Tag:</strong> ${esc(data.tag)}</div>
        </div>
      `;

      const scheduleMeta = `
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
          <div><strong>Estimated Hours:</strong> ${esc(data.estHours)}</div>
          <div><strong>Actual Hours:</strong> ${esc(data.actualHours)}</div>

          <div><strong>Estimated Cost:</strong> ${esc(data.estCost)}</div>
          <div><strong>Employee Cost:</strong> ${esc(data.employeeCost)}</div>

          <div><strong>Part Cost:</strong> ${esc(data.partCost)}</div>
          <div><strong>Other Cost:</strong> ${esc(data.otherCost)}</div>
        </div>
      `;

      const generationRules = `
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
          <div><strong>Conversion Lead Time:</strong> ${esc(data.conversionLeadTime)}</div>
          <div></div>
        </div>
      `;

      const woInfo = buildSmrWoHierarchy();

      const comments = `
        <div style="line-height:1.5;">
          ${sanitizeCommentHtml(data.comments)}
        </div>
      `;

      return {
        general: section("Overview", general),
        scheduleMeta: section("Schedule / Cost Summary", scheduleMeta),
        generationRules: section("Generation Rules", generationRules),
        woInfo: section("WO Information", woInfo),
        comments: section("Comments", comments)
      };
    }

    async function generateSmrPdf(filenameBase) {
      const reportEl = document.querySelector("#smrReportContent");
      if (!reportEl) {
        alert("Could not find SMR report content.");
        return;
      }
    
      if (typeof html2canvas === "undefined") {
        alert("html2canvas is not loaded.");
        return;
      }
    
      if (!window.jspdf || !window.jspdf.jsPDF) {
        alert("jsPDF is not loaded.");
        return;
      }
    
      const { jsPDF } = window.jspdf;
      const pdf = new jsPDF("p", "mm", "a4");
    
      const pageWidth = pdf.internal.pageSize.getWidth();
      const pageHeight = pdf.internal.pageSize.getHeight();
    
      const margin = 8;
      const usableWidth = pageWidth - margin * 2;
      const usableHeight = pageHeight - margin * 2;
    
      const clone = reportEl.cloneNode(true);
    
      clone.querySelectorAll("details").forEach(d => {
        d.open = true;
      });
    
      clone.querySelectorAll("button").forEach(btn => btn.remove());
    
      clone.style.width = "960px";
      clone.style.maxWidth = "960px";
      clone.style.margin = "0";
      clone.style.boxSizing = "border-box";
      clone.style.background = "#ffffff";
      clone.style.color = "#222";
      clone.style.overflow = "visible";
    
      clone.querySelectorAll("*").forEach(el => {
        el.style.boxSizing = "border-box";
      });
    
      clone.querySelectorAll("table").forEach(table => {
        table.style.width = "100%";
        table.style.maxWidth = "100%";
        table.style.tableLayout = "auto";
      });
    
      clone.querySelectorAll("th, td").forEach(cell => {
        cell.style.whiteSpace = "normal";
        cell.style.wordBreak = "break-word";
        cell.style.overflowWrap = "anywhere";
      });
    
      clone.querySelectorAll("summary").forEach(summary => {
        summary.style.flexWrap = "wrap";
      });
    
      const captureWrap = document.createElement("div");
      captureWrap.style.position = "absolute";
      captureWrap.style.left = "-99999px";
      captureWrap.style.top = "0";
      captureWrap.style.width = "1000px";
      captureWrap.style.padding = "20px";
      captureWrap.style.background = "#ffffff";
      captureWrap.style.overflow = "visible";
      captureWrap.style.boxSizing = "border-box";
      captureWrap.appendChild(clone);
    
      document.body.appendChild(captureWrap);
    
      try {
        const blocks = [];
    
        const header = clone.querySelector("#smrReportHeader");
        const overview = clone.querySelector("#smrOverviewSection");
        const schedule = clone.querySelector("#smrScheduleSection");
        const generationRules = clone.querySelector("#smrGenerationRulesSection");
        const comments = clone.querySelector("#smrCommentsSection");
        const taskBlocks = [...clone.querySelectorAll(".smr-task-block")];
    
        if (header) blocks.push(header);
        if (overview) blocks.push(overview);
        if (schedule) blocks.push(schedule);
        if (generationRules) blocks.push(generationRules);
    
        if (taskBlocks.length) {
          taskBlocks.forEach(task => blocks.push(task));
        } else {
          const woInfo = clone.querySelector("#smrWoInfoSection");
          if (woInfo) blocks.push(woInfo);
        }
    
        if (comments) blocks.push(comments);
    
        let y = margin;
        let hasPlacedAnything = false;
    
        for (const block of blocks) {
          const canvas = await html2canvas(block, {
            scale: 2,
            useCORS: true,
            backgroundColor: "#ffffff",
            logging: false
          });
    
          const imgData = canvas.toDataURL("image/png");
          const imgWidth = usableWidth;
          const imgHeight = (canvas.height * imgWidth) / canvas.width;
    
          // If block is taller than a page, fall back to image slicing for that block only
          if (imgHeight > usableHeight) {
            if (hasPlacedAnything) {
              pdf.addPage();
              y = margin;
            }
    
            let heightLeft = imgHeight;
            let position = y;
    
            pdf.addImage(imgData, "PNG", margin, position, imgWidth, imgHeight);
            heightLeft -= usableHeight;
            hasPlacedAnything = true;
    
            while (heightLeft > 0) {
              pdf.addPage();
              position = margin - (imgHeight - heightLeft);
              pdf.addImage(imgData, "PNG", margin, position, imgWidth, imgHeight);
              heightLeft -= usableHeight;
            }
    
            y = margin + 4;
            continue;
          }
    
          // Normal case: move block to next page if it won't fit
          if (hasPlacedAnything && (y + imgHeight > margin + usableHeight)) {
            pdf.addPage();
            y = margin;
          }
    
          pdf.addImage(imgData, "PNG", margin, y, imgWidth, imgHeight);
          y += imgHeight + 4;
          hasPlacedAnything = true;
        }
    
        pdf.save(`${filenameBase}.pdf`);
      } finally {
        document.body.removeChild(captureWrap);
      }
    }

    function showReportOverlay(html, filenameBase) {
      const overlay = document.createElement("div");
      overlay.style.cssText = `
        position:fixed; inset:0; background:rgba(0,0,0,0.45); z-index:10000;
        display:flex; align-items:center; justify-content:center; padding:20px;
      `;

      const modal = document.createElement("div");
      modal.style.cssText = `
        background:#fff; width:min(1100px,95vw); max-height:92vh;
        border-radius:12px; box-shadow:0 10px 30px rgba(0,0,0,0.25);
        overflow:hidden; display:flex; flex-direction:column; font-family:Arial,sans-serif;
      `;

      const toolbar = document.createElement("div");
      toolbar.style.cssText = `
        display:flex; align-items:center; justify-content:space-between;
        padding:12px 16px; border-bottom:1px solid #ddd; background:#f8f9fb;
      `;

      toolbar.innerHTML = `
        <div style="font-weight:700;">Report Preview</div>
        <div style="display:flex;gap:8px;">
          <button id="smrReportPdfBtn" style="padding:8px 12px;border-radius:8px;cursor:pointer;">Generate PDF</button>
          <button id="smrReportPrintBtn" style="padding:8px 12px;border-radius:8px;cursor:pointer;">Print</button>
          <button id="smrReportCloseBtn" style="padding:8px 12px;border-radius:8px;cursor:pointer;">Close</button>
        </div>
      `;

      const body = document.createElement("div");
      body.style.cssText = `padding:20px; overflow:auto; background:#fff;`;
      body.innerHTML = html;

      modal.appendChild(toolbar);
      modal.appendChild(body);
      overlay.appendChild(modal);
      document.body.appendChild(overlay);

      toolbar.querySelector("#smrReportCloseBtn").onclick = () => overlay.remove();

      toolbar.querySelector("#smrReportPrintBtn").onclick = () => {
        const win = window.open("", "_blank");
        if (!win) return;

        const printable = body.querySelector("#smrReportContent")?.cloneNode(true);
        if (printable) {
          printable.querySelectorAll("details").forEach(d => { d.open = true; });
          win.document.write(`
            <html>
              <head><title>${filenameBase}</title></head>
              <body style="font-family:Arial,sans-serif;padding:20px;background:#fff;">
                ${printable.outerHTML}
              </body>
            </html>
          `);
        } else {
          win.document.write(`
            <html>
              <head><title>${filenameBase}</title></head>
              <body style="font-family:Arial,sans-serif;padding:20px;background:#fff;">
                ${html}
              </body>
            </html>
          `);
        }

        win.document.close();
        win.focus();
        win.print();
      };

      toolbar.querySelector("#smrReportPdfBtn").onclick = async () => {
        const btn = toolbar.querySelector("#smrReportPdfBtn");
        try {
          btn.disabled = true;
          btn.textContent = "Generating...";
          await generateSmrPdf(filenameBase);
        } catch (err) {
          console.error("SMR PDF generation failed", err);
          alert("PDF generation failed. Check console for details.");
        } finally {
          btn.disabled = false;
          btn.textContent = "Generate PDF";
        }
      };
    }

    const sanitizeCommentHtml = (html) => {
      if (!html) return "—";
    
      const div = document.createElement("div");
      div.innerHTML = html;
    
      // remove risky elements
      div.querySelectorAll("script, style, iframe, object, embed").forEach(el => el.remove());
    
      // strip event handlers and javascript: URLs
      div.querySelectorAll("*").forEach(el => {
        [...el.attributes].forEach(attr => {
          const name = attr.name.toLowerCase();
          const value = String(attr.value || "");
    
          if (name.startsWith("on")) {
            el.removeAttribute(attr.name);
          }
    
          if ((name === "href" || name === "src") && value.trim().toLowerCase().startsWith("javascript:")) {
            el.removeAttribute(attr.name);
          }
        });
      });
    
      return div.innerHTML || "—";
    };
    

    async function generateSmrReport() {
      let d = window.__MPULSE_FORMVIEW__;

      if (!d) {
        console.warn("No __MPULSE_FORMVIEW__ captured yet.");
        alert("SMR data not ready yet. Refresh the record and try again.");
        return;
      }

      const reportType = await chooseReportType();
      if (!reportType) return;

      const reportConfig = REPORT_TYPES[reportType] || REPORT_TYPES.full;
      const data = getSmrData();
      const sections = buildSections(data);

      const headerHTML = buildHeader(data, reportConfig);
      const sectionHTML = reportConfig.sections
        .map(key => sections[key])
        .filter(Boolean)
        .join("");

      const reportHTML = `
        <div id="smrReportContent" style="max-width:1000px;margin:0 auto;color:#222;font-family:Arial,sans-serif;">
          <div id="smrReportHeader">
            ${headerHTML}
          </div>
      
          ${reportConfig.sections.includes("general") ? `
            <div id="smrOverviewSection">
              ${sections.general}
            </div>
          ` : ""}
      
          ${reportConfig.sections.includes("scheduleMeta") ? `
            <div id="smrScheduleSection">
              ${sections.scheduleMeta}
            </div>
          ` : ""}
      
          ${reportConfig.sections.includes("generationRules") ? `
            <div id="smrGenerationRulesSection">
              ${sections.generationRules}
            </div>
          ` : ""}
      
          ${reportConfig.sections.includes("woInfo") ? `
            <div id="smrWoInfoSection">
              ${sections.woInfo}
            </div>
          ` : ""}
      
          ${reportConfig.sections.includes("comments") ? `
            <div id="smrCommentsSection">
              ${sections.comments}
            </div>
          ` : ""}
        </div>
      `;

      const filenameBase = safeFilename(`${data.id || "SMR"}_${reportConfig.filenameSuffix}_Report`);
      showReportOverlay(reportHTML, filenameBase);
    }

    function injectButton(toolbar) {
      if (!toolbar || document.getElementById("generateSmrReportBtn")) return;
    
      // Only show on SMR records
      if (!isSmrRecord()) {
        console.log("📄 SMR Report button suppressed (not an SMR record)");
        return;
      }
    
      const li = document.createElement("li");
      li.id = "generateSmrReportBtn";
      li.className = "ng-scope";
      li.title = "Generate SMR Report";
    
      const a = document.createElement("a");
      a.style.cursor = "pointer";
    
      // Font Awesome PDF icon
      a.innerHTML = `<i class="fas fa-file-pdf"></i>`;
    
      a.onclick = async (e) => {
        e.preventDefault();
        await generateSmrReport();
      };
    
      li.appendChild(a);
    
      // prepend if you want it first, append if you want it last
      toolbar.append(li);
    }

    try {
      // Load jsPDF + html2canvas from GitHub (no CSP violation)
      if (!window.jspdf || !window.html2canvas) {
        await new Promise((res) => {
          const sc = document.createElement("script");
          sc.src = "https://cdn.jsdelivr.net/gh/dolivarez/mpulse-scripts/html2canvas.min.js";
          sc.onload = res;
          document.head.appendChild(sc);
        });

        await new Promise((res) => {
          const sc = document.createElement("script");
          sc.src = "https://cdn.jsdelivr.net/gh/dolivarez/mpulse-scripts/jspdf.umd.min.js";
          sc.onload = res;
          document.head.appendChild(sc);
        });
      }
    } catch (err) {
      console.warn("PDF libraries failed to load. Preview will still work.", err);
    }

    const toolbar = await waitForToolbar();
    injectButton(toolbar);

    const observer = new MutationObserver(() => {
      const toolbarNow = document.querySelector(".action-menu-items ul.itemDetailActionBtns");
      if (toolbarNow && !document.querySelector("#smrReportBtn")) {
        injectButton(toolbarNow);
      }
    });

    observer.observe(document.body, { childList: true, subtree: true });
  }

  init().catch(err => console.error("SMR report init failed", err));
})();
